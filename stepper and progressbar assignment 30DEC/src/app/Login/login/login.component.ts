...
import { GlobalComponent } from 'src/app/Globals';
...
export class LoginComponent implements OnInit{
  ...
  constructor(private userservice:ServicesService,private router:Router,public globals:GlobalComponent){}
  ngOnInit(): void {
  ...
  submitdata(){
    if(this.email.valid && this.password.valid){
      ...
      this.userservice.getAllUserData().subscribe({
        next:(user)=>{
          user.forEach((data)=>{
            if(this.userDetail.email==data.email&&this.userDetail.password==data.password){
              localStorage.setItem('welcomeMsg',data.fullName);
              this.loginErrorMsg==false?this.globals.IsLogin=true:'';
